...
function Table(props: TableProps) {
    ...
    const rowClicked = (identifier: number) => {
        const pcPart: ComputerComponent = DataList.filter((p) => {return p.id === identifier;})[0];
	...
	return (
		<div className="TablePanel">
			<table className="PartTable">
				...
				<tbody key={100}>
					{DataList.map((pc_part, key) => {
						return (
							<tr className="TableRow" onClick={() => rowClicked(pc_part.id)} key={key}>
								...
								<td className="TableCell" style={{ minWidth: "15%" }}>
									<div
										...
										<EditButton element_id={pc_part.id}/>
										<DeleteButton element_id={pc_part.id}/>
										<ViewButton link={"/product-" + pc_part.id}/>
