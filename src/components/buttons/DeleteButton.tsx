...
function DeleteButton(props: ButtonProps){
    ...
    const handleClose = (identifier:number, option: number) => {
      setOpen(false);
    ...
    onClick={(event) => {deleteClicked(props.element_id); event.stopPropagation(); event.preventDefault();}} 
        className="DeleteButton"><DeleteForeverIcon style={{fontSize: "1.4em"}}/>DELETE</Button>
    <Dialog
        onClick={(event) => {event.stopPropagation(); event.preventDefault();}}
        style={{bottom: "5vh"}}
          open={open}
          onClose={() => handleClose(0, 0)}
        >
            <div className="DeleteDialog">
                <DialogContent>
                    <DialogContentText style={{color: "aliceblue", fontSize: "0.6em"}} id="alert-dialog-description">Delete selected entry?</DialogContentText>
                </DialogContent>
                <DialogActions style={{justifyContent: "space-evenly"}}>
                    <Button color="buttonRed" variant="contained" 
                    onClick={(event) => {handleClose(props.element_id, 0); event.stopPropagation(); event.preventDefault();}} 
                    >No</Button>
                    <Button color="buttonGreen" variant="contained" 
                    onClick={(event) => {handleClose(props.element_id, 1); event.stopPropagation(); event.preventDefault();}} 
